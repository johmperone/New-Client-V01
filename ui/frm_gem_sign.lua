...
function layWorld_frm_StoneSign_OnLoad(self)
	self:RegisterScriptEventNotify("EVENT_LocalGurl");
end

function layWorld_frm_StoneSign_OnEvent(self, event, args)
	if event == "EVENT_LocalGurl" then
		local address = args[1];
		if address == "signstone" then
			self:ShowAndFocus();
		end
	end
end

function layWorld_frm_StoneSign_btItem_OnLoad(self)
	self:Set(EV_UI_SHORTCUT_OWNER_KEY, EV_UI_SHORTCUT_OWNER_SIGN);
end

function layWorld_frm_StoneSign_btItem_Clear(self)
	...
	layWorld_frm_StoneSign_btItem_Refresh(self);
end

function layWorld_frm_StoneSign_btItem_Refresh(self)
	local shortcut_owner = self:Get(EV_UI_SHORTCUT_OWNER_KEY);
	if shortcut_owner == nil or shortcut_owner ~= EV_UI_SHORTCUT_OWNER_SIGN then return end
...
function layWorld_frm_StoneSign_OnShow(self)
	uiRegisterEscWidget(self);
end

function layWorld_frm_StoneSign_OnHide(self)
	local btItem = SAPI.GetChild(self, "btItem");
	layWorld_frm_StoneSign_btItem_Clear(btItem);
end

function layWorld_frm_StoneSign_btItem_OnDragIn(self, drag)
	local allow_owners = 
	{
	...
	-- 检查 是否可以签名	
	if uiItemStoneCanSigned(shortcut_objectid) ~=true then return end
	
	-- 检查通过,可以放进来
	layWorld_frm_StoneSign_btItem_Clear(self); -- 先把旧的清除掉
...
function layWorld_frm_StoneSign_btItem_OnDragNull(self)
	layWorld_frm_StoneSign_btItem_Clear(self);
end

function layWorld_frm_StoneSign_btItem_OnRClick(self)
	layWorld_frm_StoneSign_btItem_Clear(self);
end

function layWorld_frm_StoneSign_btItem_OnHint(self)
	local hint = 0;
	local shortcut_type = self:Get(EV_UI_SHORTCUT_TYPE_KEY);
...
function layWorld_frm_StoneSign_btOK_OnLClick(self)
	-- 执行签名操作
	...
	if uiItemStoneSigned(objectid) == true then
		layWorld_frm_StoneSign_btItem_Clear(btItem);
	end
end

function layWorld_frm_StoneSign_btCancel_OnLClick(self)
	local frame = SAPI.GetParent(self);
	frame:Hide();
end

function layWorld_frm_StoneSign_OnDragIn( self,drag )
	layWorld_frm_StoneSign_btItem_OnDragIn( SAPI.GetChild(self,"btItem"), drag )
end
