...
function layWorld_frmSignDelete_OnLoad(self)
	self:RegisterScriptEventNotify("EVENT_LocalGurl");
end

function layWorld_frmSignDelete_OnEvent(self, event, args)
	if event == "EVENT_LocalGurl" then
		local address = args[1];
		if address == "unsign" then
			self:ShowAndFocus();
		end
	end
end

function layWorld_frmSignDelete_btItem_OnLoad(self)
	self:Set(EV_UI_SHORTCUT_OWNER_KEY, EV_UI_SHORTCUT_OWNER_SIGN);
end

function layWorld_frmSignDelete_btItem_Clear(self)
	...
	layWorld_frmSignDelete_btItem_Refresh(self);
end

function layWorld_frmSignDelete_btItem_Refresh(self)
	local shortcut_owner = self:Get(EV_UI_SHORTCUT_OWNER_KEY);
	if shortcut_owner == nil or shortcut_owner ~= EV_UI_SHORTCUT_OWNER_SIGN then return end
...
function layWorld_frmSignDelete_OnShow(self)
	uiRegisterEscWidget(self);
end

function layWorld_frmSignDelete_OnHide(self)
	local btItem = SAPI.GetChild(self, "btItem");
	layWorld_frmSignDelete_btItem_Clear(btItem);
end

function layWorld_frmSignDelete_btItem_OnDragIn(self, drag)
	local allow_owners = 
	{
	...
	-- 检查 是否可以签名
	if uiItemCanUnsigned(shortcut_objectid, true) ~= true then return end
...
function layWorld_frmSignDelete_btItem_OnDragNull(self)
	layWorld_frmSignDelete_btItem_Clear(self);
end

function layWorld_frmSignDelete_btItem_OnRClick(self)
	layWorld_frmSignDelete_btItem_Clear(self);
end

function layWorld_frmSignDelete_btItem_OnHint(self)
	local hint = 0;
	local shortcut_type = self:Get(EV_UI_SHORTCUT_TYPE_KEY);
...
function layWorld_frmSignDelete_btOK_OnLClick(self)
	-- 执行签名操作
	...
	if uiItemUnsigned(objectid) == true then
		layWorld_frmSignDelete_btItem_Clear(btItem);
	end
end

function layWorld_frmSignDelete_btCancel_OnLClick(self)
